...
class LifeGate(core.Env):
    def __init__(self, state_mode, rng, death_drag, cont_states=False, max_steps=100, fixed_life=True, rendering=False, image_saving=False, render_dir=None):
        ...
        self.cont_states = cont_states  # Whether we'll return a continuous version of the tabular (x,y) coordinate
                                        # Done by adding Gaussian noise to the observations
        # self.scr_w = None
        # self.scr_h = None
    ...
    def init_episode(self):
        ...
        self.player_pos_x = np.random.randint(0, 4)  # Randomly set x-position anywhere but on the terminal edge
        # self.player_pos_y = np.random.randint(1, self.scr_h)    # Randomly set y-position anywhere but on the top edge (where goal is)
        self.player_pos_y = np.random.randint(6, self.scr_h)    # Randomly set y-position anywhere in the bottom half of the frame
        # self.player_pos_x, self.player_pos_y = 2, self.scr_h - 1
        targets = deepcopy(self.possible_recoveries)
        self.recoveries = targets
    ...
    def render(self):
        ...
        # for pos in self.risk_zone:
        #     p = [self.rendering_scale * pos[0], self.rendering_scale * pos[1]]
        #     rec1 = pygame.Rect(p[0], p[1], size[0], size[1])
        #     pygame.draw.rect(self.screen, YL_RD, rec1)
    ...
    def _move_player(self, action):
        ...
        if [x, y] in self.dead_ends:
            if self.rng.binomial(1, 0.70):
                ...
                action = 0  # no-op
        # elif [x, y] in self.risk_zone:
        #     # Half the time, move randomly according to some 
        #     # set distribution biased toward no-op and left
        #     if self.rng.binomial(1, 0.50): 
        #         action = self.rng.choice(range(5), p=[.4, .15, .05, .35, .05])
        else:
...
class LifeGatePivot(LifeGate):
    def init_subclass(self):
        ...
        self.possible_recoveries = [[5,0], [6, 0], [7, 0]]
        self.main_deaths = [[self.scr_w - 1, k] for k in range(self.scr_h)]+ [[8,0]]
        # self.dead_ends = [[x, y] for x in range(self.scr_w // 2 + 1, self.scr_w - 1) for y in range(3*self.scr_w // 4, self.scr_w)]
        # self.risk_zone = [[x, y] for x in range(self.scr_w//2, self.scr_w-1) for y in range(self.scr_w//2, self.scr_w) if [x, y] not in self.dead_ends]
        self.dead_ends = [[x, y] for x in range(self.scr_w // 2, self.scr_w - 1) for y in range(self.scr_w // 2, self.scr_w)]
        self.observability_switch_point = [0, 5]
