...
class IQN_Agent():
    ...
    def __init__(self,
        ...
        """
        self.state_size = state_size  # Dimension of the state observations
        self.action_size = action_size  # Dimension of the action space (discrete)
        self.network = network  # Description of the components put together for the IQN model training.
        self.munchausen = munchausen  # Whether we are using Munchausen RL during training.
        self.seed = random.seed(seed)  # The specified Random seed for reproducibility
        self.seed_t = torch.manual_seed(seed)
        self.device = device  # The device that models and data will be loaded to
        self.drm = risk_measure  # The distortion risk measure that will be used to transform sample distribution from Value functions
        self.sided_Q = sided_Q  # Whether we'll 
        self.ETA = ETA  # The scaling factor for the Distortion Risk Measure
        self.TAU = TAU  # The weight of the Polyak average between the local and target networks
        self.N = N  # The number of samples to draw from the value distributions when training
        self.K = 32  # The number of samples to draw from the value distribution when estimating the value function for the policy.
        self.entropy_tau = 0.03  # For Munchausen RL
        ...
        # IQN-Network
        self.qnetwork_local = IQN(state_size, action_size, layer_size, n_step, seed, N, drm=risk_measure, dueling=duel, noisy=noisy, device=device).to(device)
        self.qnetwork_target = IQN(state_size, action_size, layer_size, n_step, seed, N, drm=risk_measure, dueling=duel, noisy=noisy, device=device).to(device)
    ...
    def act(self, state, eps=0., eval=False, use_drm=False):
        ...
        if random.random() > eps: # select greedy action if random number is higher than epsilon or noisy network is used!
            ...
            with torch.no_grad():
                action_values = self.qnetwork_local.get_qvalues(state, act=True, use_drm=use_drm)#.mean(0)
    ...
    def learn(self, experiences):
        ...
        if not self.munchausen:
            ...
            if self.sided_Q == 'negative':
                Q_targets = rewards.unsqueeze(-1) + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=0.0, min=-1.0) * (1. - dones.unsqueeze(-1)))
            elif self.sided_Q == 'positive':
                Q_targets = rewards.unsqueeze(-1) + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=1.0, min=0.0) * (1. - dones.unsqueeze(-1)))
            else:
                Q_targets = rewards.unsqueeze(-1) + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=1.0, min=-1.0) * (1. - dones.unsqueeze(-1)))
...
class DQN_Agent():
    """Interacts with and learns from the environment. Implements a standard Double DQN agent."""
    ...
    def __init__(self,
        ...
        # DQN-Network
        self.qnetwork_local = DQN(state_size, action_size, layer_size, seed, dueling=duel, noisy=noisy, device=device).to(device)
        self.qnetwork_target = DQN(state_size, action_size, layer_size, seed, dueling=duel, noisy=noisy, device=device).to(device)
    ...
    def learn(self, experiences):
        ...
        if not self.munchausen:
            ...
            Q_targets_next = Q_targets_next.gather(1, action_indx)
                        # Compute Q targets for current states, clamp Q_values depending on what 
            if self.sided_Q == 'negative':
                Q_targets = rewards + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=0.0, min=-1.0) * (1. - dones))
            elif self.sided_Q == 'positive':
                Q_targets = rewards + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=1.0, min=0.0) * (1. - dones))
            else:
                Q_targets = rewards + (self.GAMMA**self.n_step * torch.clamp(Q_targets_next.to(self.device), max=1.0, min=-1.0) * (1. - dones))
            # Get expected Q values from local model
            ...
            # Huber loss
            td_error = Q_targets - Q_expected
            assert td_error.shape == (self.BATCH_SIZE, 1), "wrong td error shape"
            huber_l = calculate_huber_loss(td_error, 1.0)
            ...
            states, actions, rewards, next_states, dones = experiences
            Q_targets_next= self.qnetwork_target(next_states)
            q_t_n = Q_targets_next.detach() #(batch, actions)
            ...
            Q_expected = q_k.gather(2, actions.unsqueeze(-1).expand(self.BATCH_SIZE, 1))
    ...
    def learn_per(self, experiences):
            ...
            if not self.munchausen:
                ...
                Q_targets_next = Q_targets_next.detach().cpu()
                action_indx = torch.argmax(Q_targets_next.mean(dim=1), dim=1, keepdim=True)
                Q_targets_next = Q_targets_next.gather(2, action_indx.unsqueeze(-1).expand(self.BATCH_SIZE, 1)).transpose(1,2)
                # Compute Q targets for current states 
                Q_targets = rewards.unsqueeze(-1) + (self.GAMMA**self.n_step * Q_targets_next.to(self.device) * (1. - dones.unsqueeze(-1)))
                # Get expected Q values from local model
                Q_expected = self.qnetwork_local(states)
                Q_expected = Q_expected.gather(2, actions.unsqueeze(-1).expand(self.BATCH_SIZE, 1))
                ...
                logsum = torch.logsumexp(\
                    (Q_targets_next - Q_targets_next.max(2)[0].unsqueeze(-1))/self.entropy_tau, 2).unsqueeze(-1) #logsum trick
                assert logsum.shape == (self.BATCH_SIZE, 1), "log pi next has wrong shape"
